...
from NOCS_pose_error import compute_error_degree_cm_transform
...
def decompose_NOCS_GT_transform_matrix(gt_RT_matrix): 
    # decompose both pred, and gt, into rotation, translation, scale. And calculate based on the decomposed components
    # 1) examples (pred, gt pairs, whether agree with the code of NOCS), 2) curve 
    assert (gt_RT_matrix.shape == (4, 4))
    # get translation first
    translation = gt_RT_matrix[0:3, 3]
    # get scale & rotation
    scale = np.array([1.0, 1.0, 1.0])
    rotation = np.ones((3, 3), dtype = gt_RT_matrix.dtype)
    for i in range(3):
        norm = np.linalg.norm(gt_RT_matrix[:, i])
        scale[i] = norm
        rotation[0:3,i] = gt_RT_matrix[0:3,i] / norm
        #check rotation is with det()=1 or not
    return translation, scale, rotation

def compute_RT_degree_cm_symmetry_decompose(RT_1, RT_2, class_id, handle_visibility, synset_names):
    '''
    :param RT_1: [4, 4]. homogeneous affine transformation
    :param RT_2: [4, 4]. homogeneous affine transformation
    :return: theta: angle difference of R in degree, shift: l2 difference of T in centimeter
    ...
    '''
    # print('decomposed RT degree cm symmetry decompose are called')
    ## make sure the last row is [0, 0, 0, 1]
    if RT_1 is None or RT_2 is None:
        return -1
    try:
    ...
    T1, _, R1 = decompose_NOCS_GT_transform_matrix(RT_1)
    T2, _, R2 = decompose_NOCS_GT_transform_matrix(RT_2)
...
def compute_RT_overlaps_decompose(gt_class_ids, gt_RTs, gt_handle_visibility,
                        pred_class_ids, pred_RTs, 
                        synset_names):
    """Finds overlaps between prediction and ground truth instances.
    Returns:
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j, :] = compute_error_degree_cm_transform(pred_RTs[i], 
                                                              gt_RTs[j], 
                                                              synset_names[gt_class_ids[j]], 
                                                              gt_handle_visibility[j], False)
            
    return overlaps

def compute_degree_cm_mAP_decompose(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    """Compute Average Precision at a set IoU threshold (default 0.5).
    Returns:
    mAP: Mean Average Precision
    precisions: List of precisions at different class score thresholds.
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            RT_overlaps = compute_RT_overlaps_decompose(cls_gt_class_ids, cls_gt_RTs, cls_gt_handle_visibility, 
                                              cls_pred_class_ids, cls_pred_RTs,
                                              synset_names)
    ...
    # draw pose AP vs. thresholds
    if use_matches_for_pose:
        prefix='Ours_Pose_Only_'
    else:
        prefix='Ours_Pose_Detection_'
